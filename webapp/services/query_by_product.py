...
#Filtra o dataframe por produto (derivados) e retorna o mesmo
def query_by_product_combs(product : str) -> List[dict]:
    ...
    df = df_combs_clean[df_combs_clean['product'].str.contains(f"{product}")]
    if df.empty:
        return {'Message' : 'Nenhum item encontrado com o produto enviado'}
    return df.to_dict(orient='records')
    

#Filtra o dataframe por produto (diesel) e retorna o mesmo
def query_by_product_diesel(product : str) -> List[dict]:
    ...
    product = product.upper()
    df = df_diesel_clean[df_diesel_clean['product'].str.contains(f"{product}")]
    if df.empty:
        return {'Message' : 'Nenhum item encontrado com o produto enviado'}
    return df.to_dict(orient='records')